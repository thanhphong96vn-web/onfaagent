  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (sentMessage && sentMessage.id) {
        console.log(`✅ WhatsApp Web message sent to ${chatId} (ID: ${sentMessage.id._serialized || sentMessage.id})`);
        return true;
      } else {
      ...
      if (attempt === maxRetries) {
        console.error(`❌ Error sending WhatsApp Web message to ${chatId} after ${maxRetries} attempts:`, error);
        throw error;
      }
